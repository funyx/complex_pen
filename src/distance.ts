import { GeoLocation, Partner } from "./interface"
 ...
 */
export function distance<Type = GeoLocation>(origin: GeoLocation | Partner, target: GeoLocation | Partner, earthRadius: number = 6371.009): {
    distance: number,
    heading: number,
} {
 ...
 * default value  sphere with this radius results in an error of up to about 0.5%.
 */
export function inCircle<Type = GeoLocation>(origin: GeoLocation | Partner, targets: GeoLocation[] | Partner[], radius: number, earthRadius: number = 6371.009): Type[] {
    return targets.filter((target) => distance(origin, target, earthRadius).distance <= radius) as Type[]
}